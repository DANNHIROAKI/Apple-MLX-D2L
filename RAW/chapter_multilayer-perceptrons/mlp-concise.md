...
class MLP(nn.Module):
    def __init__(self):
        ...
        self.layers = [
            nn.Sequential(lambda x: mx.array.flatten(x, start_axis=1, end_axis=-1)),
            nn.Linear(784, 256),
            nn.Linear(256, 10)
        ]
...
for i in range(1, len(net.layers)):
    net.layers[i].weight = weight_fn(params["layers"][i]["weight"])
    net.layers[i].bias = bias_fn(params["layers"][i]["bias"])
```
...
```{.python .input}
#@tab pytorch,mxnet,tensorflow,paddle
