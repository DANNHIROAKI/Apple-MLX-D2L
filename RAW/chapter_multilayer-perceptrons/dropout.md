...
net = twoHidden_MLP(num_inputs, num_outputs, num_hiddens1, num_hiddens2)
```
...
class twoHidden_MLP(nn.Module):
    def __init__(self, num_inputs, num_outputs, num_hiddens1, num_hiddens2):
        super().__init__()
        self.layers = [nn.Sequential(lambda x: mx.array.flatten(x, start_axis=1, end_axis=-1)),
