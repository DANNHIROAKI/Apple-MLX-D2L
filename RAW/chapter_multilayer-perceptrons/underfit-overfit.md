...
```{.python .input}
#@tab mlx
# NumPy ndarray转换为mx.array
true_w, features, poly_features, labels = [mx.array(x, dtype=
    mx.float32) for x in [true_w, features, poly_features, labels]]
```
...
class simpleNet(nn.Module):
    def __init__(self, input_shape, output_shape):
        ...
        self.layers = [nn.Linear(input_shape, output_shape, bias=False)]
...
def train(train_features, test_features, train_labels, test_labels,
    ...
    input_shape = train_features.shape[-1]
    # 不设置偏置，因为我们已经在多项式中实现了它
    net = simpleNet(input_shape, 1)
    batch_size = min(10, train_labels.shape[0])

    trainer = optim.SGD(learning_rate=0.01)
    animator = d2l.Animator(xlabel='epoch', ylabel='loss', yscale='log',
                            xlim=[1, num_epochs], ylim=[1e-3, 1e2],
                            legend=['train', 'test'])
    for epoch in range(num_epochs):
        d2l.train_epoch_ch3(net, d2l.load_array((train_features, train_labels.reshape(-1,1)),
                                batch_size), loss, trainer, batch_size, params=None)
        if epoch == 0 or (epoch + 1) % 20 == 0:
            animator.add(epoch + 1, (evaluate_loss(net, d2l.load_array((train_features, train_labels.reshape(-1,1)),
                                batch_size), loss, params=None),
                                    evaluate_loss(net, d2l.load_array((test_features, test_labels.reshape(-1,1)),
                               batch_size, is_train=False), loss, params=None)))
    print('weight:', net.layers[0].weight)
```
