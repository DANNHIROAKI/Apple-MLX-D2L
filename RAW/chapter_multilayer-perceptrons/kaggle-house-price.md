...
# “Dummy_na=True”将“na”（缺失值）视为有效的特征值，并为其创建指示符特征
all_features = pd.get_dummies(all_features, dummy_na=True)
all_features = all_features.to_numpy().astype(np.float32)
all_features = mx.array(all_features)
all_features.shape
```
...
#@tab mlx
n_train = train_data.shape[0]
train_features = d2l.tensor(all_features[:n_train], dtype=d2l.float32)
test_features = d2l.tensor(all_features[n_train:], dtype=d2l.float32)
train_labels = d2l.tensor(
    train_data.SalePrice.values.reshape(-1, 1), dtype=d2l.float32)
```
...
def log_rmse(net, features, labels):
    # 为了在取对数时进一步稳定该值，将小于1的值设置为1
    clipped_preds = mx.clip(net(features), 1, float('inf'))
    rmse = mx.sqrt(loss(mx.log(clipped_preds),
                           mx.log(labels)))
    return rmse.item()
```
...
def train(net, train_features, train_labels, test_features, test_labels,
    ...
    # train_iter = d2l.load_array((train_features, train_labels), batch_size)
    # 这里使用的是Adam优化算法
    updater = optim.AdamW(learning_rate = learning_rate,
                            weight_decay = weight_decay)
    for epoch in range(num_epochs):
        for X, y in d2l.load_array((train_features, train_labels), batch_size):
...
def train_and_pred(train_features, test_features, train_labels, test_data,
    ...
    print(f'训练log rmse：{float(train_ls[-1]):f}')
    # 将网络应用于测试集。
    preds = np.array(net(test_features))
    # 将其重新格式化以导出到Kaggle
    test_data['SalePrice'] = pd.Series(preds.reshape(1, -1)[0])
    submission = pd.concat([test_data['Id'], test_data['SalePrice']], axis=1)
    submission.to_csv('submission.csv', index=False)
