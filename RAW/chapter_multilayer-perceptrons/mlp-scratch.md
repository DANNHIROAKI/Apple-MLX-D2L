...
def net(X, params):
    X = d2l.reshape(X, (-1, num_inputs))
    H = mx.matmul(X, params[0]) + params[1]
    return mx.matmul(H, params[2]) + params[3]
```
...
#@tab mlx
def loss(y_hat, y):
    return nn.losses.cross_entropy(y_hat, y)
```
...
def updater(grads, batch_size):
    return d2l.sgd(params, grads, lr, batch_size)
...
```{.python .input}
#@tab mxnet, paddle, tensorflow, pytorchh
